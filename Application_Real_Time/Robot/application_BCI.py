...
base_path = Path.cwd().parent  /"Biosignals"/"Application_Real_Time"/ "Robot" / "cur_rec"   
...
while True:
    try:
        print("waiting for EEG rec, press key to continue...")
        input()
        ...
        col_prob = y_probs[:3]
        row_prob = y_probs[3:]
        col_max = np.argmax(col_prob)
        row_max = np.argmax(row_prob)
        field_id = (col_max+1) + row_max * 3
