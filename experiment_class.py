...
class Experiment:
    ...
    def __init__(self, num_blocks=3, num_epochs=12, flashing_duration=0.1,
    ...
    def stream_eeg(self, stop_event=None):
        ...
        try:
            while not (stop_event and stop_event.is_set()):
                ...
                if eeg_sample:
                    samples.append([eeg_ts] + eeg_sample)
        finally:
            output_path = self.eeg_csv_path
            with open(output_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['timestamp'] + [f'channel_{i}' for i in range(len(samples[0]) - 1)])
                ...
                while True:
                    ...
                    if marker_sample is not None:
                        if stimulus_list:
                            ...
                            stim_rec.append([trial, marker_ts, target_idx, idx, is_target]) #only record marker when stimulus is displayed, only time and idx of interest
                        else:
